...
function Create-SharePointServiceAccounts{
    ...
    # create unattended user service account 
    $UserName = "SP_Unattended"
    Write-Host (" - adding User: {0}" -f $UserName)
    New-ADUser -Path $ouWingtipServiceAccountsPath -SamAccountName $UserName -Name $UserName -DisplayName $UserName -AccountPassword $UserPassword -Enabled $true -ChangePasswordAtLogon $false -PasswordNeverExpires $true
    # add sp_workflow account to local Administrators group
    $user_unattended = Get-ADUser -Filter "samAccountName -eq 'SP_Unattended'"
    Add-ADGroupMember -Identity "Administrators" -Members $user_unattended 


    # create access services account 
    $UserName = "SP_Access"
    Write-Host (" - adding User: {0}" -f $UserName)
    New-ADUser -Path $ouWingtipServiceAccountsPath -SamAccountName $UserName -Name $UserName -DisplayName $UserName -AccountPassword $UserPassword -Enabled $true -ChangePasswordAtLogon $false -PasswordNeverExpires $true
    # add sp_access account to local Administrators group
    #$user_access = Get-ADUser -Filter "samAccountName -eq 'SP_Access'"
    #Add-ADGroupMember -Identity "Administrators" -Members $user_access 
